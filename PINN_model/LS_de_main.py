...
data_all = np.load(os.path.join(current_dir,"..","data/LS_log_profiles.npy"))
...
t_data_all = np.load(os.path.join(current_dir,"..","data/LS_difference_times.npy"))
...
time_lbc_start = 20.0/t_data_max_all                 ########Note that we only reliably know of a fixed lbc from this time onwards. done by observing experimental profiles
lbc_value = np.mean(s_data_all[0,8:])
print("lbc value:", lbc_value)
lbc_std = np.std(s_data_all[0,8:])
...
save_dir = os.path.join(current_dir,"LS_outputs")
...
for ens in range(1,751):
    ...
    np.save(os.path.join(save_dir,f'LS_best_{ens}.npy'),best_logs)
    np.save(os.path.join(save_dir,f'LS_logs_{ens}.npy'),loss_logs)
