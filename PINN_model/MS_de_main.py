...
# Specify which GPU index to use (e.g., 0 for GPU 0)
gpu_index = 0
...
if gpus:
    try:
        # Restrict TensorFlow to only see the specified GPU
        tf.config.set_visible_devices(gpus[gpu_index], 'GPU')
        # Enable memory growth on the specified GPU
        tf.config.experimental.set_memory_growth(gpus[gpu_index], True)
        print(f"Memory growth enabled for GPU {gpu_index}.")
...
data_all = np.load(os.path.join(current_dir,"..","data/MS_log_profiles.npy"))
data_all = data_all * 0.805  # scaling is applied to match with the gravimetric absorbed water content at lower boundary 
# print(data_all.shape)


pixel_size = 31.64 #micrometers um- determined from the X-ray setting/radiograph metadata
...
t_data_all = np.load(os.path.join(current_dir,"..","data/MS_difference_times.npy"))
...
# Select only certain ones for trianing leaving 20% holdout 
##########################################################
train_idx = [0, 1, 2, 3, 5, 6, 7, 8, 9, 11, 12, 14, 15, 16, 17, 19]

t_data_scaled_all = t_data_scaled_all[train_idx]

data_all = data_all[train_idx, :]
...
time_lbc_start = 15.0/t_data_max_all                 ########Note that we only reliably know of a fixed lbc from this time onwards. done by observing experimental profiles
lbc_value = np.mean(s_data_all[0,6:])
print("lbc value:", lbc_value)
lbc_std = np.std(s_data_all[0,6:])
...
save_dir = os.path.join(current_dir,"MS_outputs")
...
for ens in range(1,751):
    ...
    np.save(os.path.join(save_dir,f'MS_best_{ens}.npy'),best_logs)
    np.save(os.path.join(save_dir,f'MS_logs_{ens}.npy'),loss_logs)
